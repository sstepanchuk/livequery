...
fn perf_snapshot_diff_100_rows() {
    ...
    let rows: Vec<_> = (0..100)
        .map(|i| row(json!({"id": i, "name": format!("User {}", i)})))
        .collect();
    s.init_rows(rows.clone(), &c);

    let mut new_rows: Vec<_> = (0..100)
        .map(|i| row(json!({"id": i, "name": format!("User {}", i)})))
        .collect();
    for i in 0..10 {
        new_rows[i] = row(json!({"id": i, "name": format!("Updated {}", i)}));
    }

    let start = Instant::now();
    let iterations = 1000;
    for _ in 0..iterations {
        let _ = s.diff_rows(new_rows.clone(), &c);
    }
    ...
    println!(
        "\n[BENCH] snapshot_diff_100_rows: {} ns/op ({:.2} µs)",
        per_op,
...
fn perf_snapshot_diff_1000_rows() {
    ...
    for i in 0..50 {
        new_rows[i] = row(
            json!({"id": i, "name": format!("Updated {}", i), "email": format!("updated{}@test.com", i)}),
        );
    }
    ...
    println!(
        "\n[BENCH] snapshot_diff_1000_rows: {} ns/op ({:.2} µs)",
        per_op,
        per_op as f64 / 1000.0
    );
}
...
fn perf_subscribe_shared_query() {
    ...
    let _ = m
        .subscribe("sub-0", q, None, SubscriptionMode::Events)
        .unwrap();
    ...
    assert!(
        ...
        "subscribe too slow: {:?} ({} ns/op)",
        elapsed,
        per_op
    );
    println!(
        "\n[BENCH] subscribe_shared_query: {} ns/op ({:.2} µs)",
        per_op,
        per_op as f64 / 1000.0
    );
}
...
fn perf_query_analyze_cached() {
    ...
    assert!(
        elapsed.as_millis() < 200,
        "analyze cache too slow: {:?} ({} ns/op)",
        elapsed,
        per_op
    );
    println!(
        "\n[BENCH] query_analyze_cached: {} ns/op ({:.2} µs)",
        per_op,
        per_op as f64 / 1000.0
    );
}
...
fn perf_event_batch_serialize() {
    let events: Vec<_> = (0..100)
        .map(|i| event::SubscribeEvent::insert(i, json!({"id": i, "name": format!("User {}", i)})))
        .collect();
    ...
    println!(
        "\n[BENCH] event_batch_serialize_100: {} ns/op ({:.2} µs)",
        per_op,
        per_op as f64 / 1000.0
    );
}
