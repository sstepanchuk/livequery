...
use deadpool_postgres::{Config, Pool, Runtime, ManagerConfig, RecyclingMethod, PoolConfig};
...
static COL_NAMES: std::sync::LazyLock<dashmap::DashMap<String, Arc<str>, rustc_hash::FxBuildHasher>> =
    std::sync::LazyLock::new(|| dashmap::DashMap::with_hasher(rustc_hash::FxBuildHasher::default()));
...
fn row_to_typed(row: &Row, cols: Arc<[Arc<str>]>, col_types: &[Type], use_index: bool) -> RowData {
    ...
    for (i, col_type) in col_types.iter().enumerate() {
        let v = match *col_type {
            Type::INT2 => row.try_get::<_, Option<i16>>(i).ok().flatten().map(|n| RowValue::Int(n as i64)),
            Type::INT4 => row.try_get::<_, Option<i32>>(i).ok().flatten().map(|n| RowValue::Int(n as i64)),
            Type::INT8 => row.try_get::<_, Option<i64>>(i).ok().flatten().map(RowValue::Int),

            Type::FLOAT4 => row.try_get::<_, Option<f32>>(i).ok().flatten().map(|f| RowValue::Float(f as f64)),
            Type::FLOAT8 => row.try_get::<_, Option<f64>>(i).ok().flatten().map(RowValue::Float),

            Type::BOOL => row.try_get::<_, Option<bool>>(i).ok().flatten().map(RowValue::Bool),

            Type::TEXT | Type::VARCHAR | Type::BPCHAR | Type::NAME | Type::NUMERIC =>
                row.try_get::<_, Option<String>>(i).ok().flatten().map(|s| RowValue::from_str(&s)),

            Type::UUID => row.try_get::<_, Option<uuid::Uuid>>(i).ok().flatten()
                .map(|u| RowValue::from_str(&u.to_string())),

            Type::JSON | Type::JSONB => row.try_get::<_, Option<Value>>(i).ok().flatten().map(RowValue::Json),

            Type::TIMESTAMP => row.try_get::<_, Option<chrono::NaiveDateTime>>(i).ok().flatten()
                .map(|t| RowValue::from_str(&t.to_string())),
            Type::TIMESTAMPTZ => row.try_get::<_, Option<chrono::DateTime<chrono::Utc>>>(i).ok().flatten()
                .map(|t| RowValue::from_str(&t.to_rfc3339())),
            Type::DATE => row.try_get::<_, Option<chrono::NaiveDate>>(i).ok().flatten()
                .map(|d| RowValue::from_str(&d.to_string())),
            Type::TIME | Type::TIMETZ => row.try_get::<_, Option<chrono::NaiveTime>>(i).ok().flatten()
                .map(|t| RowValue::from_str(&t.to_string())),

            Type::BYTEA => row.try_get::<_, Option<Vec<u8>>>(i).ok().flatten().map(RowValue::Bytes),

            Type::BOOL_ARRAY => row.try_get::<_, Option<Vec<bool>>>(i).ok().flatten().map(|a| {
                let mut out = Vec::with_capacity(a.len());
                for v in a { out.push(RowValue::Bool(v)); }
                RowValue::Array(out)
            }),
            Type::INT4_ARRAY => row.try_get::<_, Option<Vec<i32>>>(i).ok().flatten().map(|a| {
                let mut out = Vec::with_capacity(a.len());
                for n in a { out.push(RowValue::Int(n as i64)); }
                RowValue::Array(out)
            }),
            Type::INT8_ARRAY => row.try_get::<_, Option<Vec<i64>>>(i).ok().flatten().map(|a| {
                let mut out = Vec::with_capacity(a.len());
                for n in a { out.push(RowValue::Int(n)); }
                RowValue::Array(out)
            }),
            Type::TEXT_ARRAY | Type::VARCHAR_ARRAY => row.try_get::<_, Option<Vec<String>>>(i).ok().flatten().map(|a| {
                let mut out = Vec::with_capacity(a.len());
                for s in a { out.push(RowValue::from_str(&s)); }
                RowValue::Array(out)
            }),

            _ => row.try_get::<_, Option<String>>(i).ok().flatten().map(|s| RowValue::from_str(&s)),
        };
...
impl DbPool {
    ...
    pub async fn query_rows_typed(&self, q: &str) -> Result<Vec<RowData>> {
        ...
        match result {
            Ok(rows) => {
                if ms > 100 { debug!("Slow query {}ms: {:.60}", ms, q); }
    ...
    pub fn query_stats(&self) -> (u64, u64, u64) {
        ...
        let avg = if q > 0 { self.total_ms.load(Relaxed) / q } else { 0 };
        (q, e, avg)
    }
}
