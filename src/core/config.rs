...
impl Config {
    ...
    pub fn from_env() -> Result<Self> {
        let cfg = Self {
            ...
            log_level: var("LOG_LEVEL").or_else(|_| var("RUST_LOG")).unwrap_or_else(|_| "info".into()),
            shutdown_timeout_secs: env("SHUTDOWN_TIMEOUT_SECS", 30),
            
            // Database
            db_url: var("DATABASE_URL").or_else(|_| var("POSTGRES_URL")).context("DATABASE_URL required")?,
    ...
    fn validate(&self) -> Result<()> {
        macro_rules! check {
            ($cond:expr, $msg:expr) => { if $cond { bail!($msg); } };
        }
        ...
        check!(self.client_timeout_secs < 5, "CLIENT_TIMEOUT_SECS must be >= 5");
        check!(self.cleanup_interval_secs == 0, "CLEANUP_INTERVAL_SECS must be > 0");
        check!(self.max_subscriptions == 0, "MAX_SUBSCRIPTIONS must be > 0");
        check!(self.shutdown_timeout_secs == 0, "SHUTDOWN_TIMEOUT_SECS must be > 0");
        Ok(())
    }
    
    // === Duration helpers ===
    
    #[inline] pub fn client_timeout(&self) -> Duration { Duration::from_secs(self.client_timeout_secs) }
    #[inline] pub fn cleanup_interval(&self) -> Duration { Duration::from_secs(self.cleanup_interval_secs) }
    #[inline] pub fn db_timeout(&self) -> Duration { Duration::from_secs(self.db_timeout_secs) }
    #[inline] pub fn shutdown_timeout(&self) -> Duration { Duration::from_secs(self.shutdown_timeout_secs) }
    
    /// Subscription-specific events subject: livequery.{sub_id}.events
    #[inline] pub fn sub_events_subject(&self, sub_id: &str) -> String { format!("{}.{}.events", self.nats_prefix, sub_id) }
    
    /// Safe URL for logging (hides password)
    pub fn db_url_safe(&self) -> String {
        self.db_url.find('@').and_then(|a| {
            self.db_url[..a].rfind(':').map(|c| format!("{}****{}", &self.db_url[..c+1], &self.db_url[a..]))
    ...
    pub fn log_summary(&self) {
        ...
        info!("│ timeout:   {}s cleanup={}s{:<13} │", self.client_timeout_secs, self.cleanup_interval_secs, "");
        info!("╰────────────────────────────────────────╯");
    }
}
