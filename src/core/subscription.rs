...
use crate::core::event::{SubscribeEvent, EventBatch, SubscriptionMode, ts_millis};
...
pub struct Snapshot { rows: FxHashMap<u64, RowEntry> }
...
fn row_hashes(r: &RowData, cols: &Option<Arc<[Arc<str>]>>) -> (u64, u64) {
    ...
    match cols {
        ...
        Some(c) => {
            ...
            if c.len() == 1 {
                if let Some(v) = r.get(&c[0]) { v.hash_into(&mut h); }
            } else {
                for col in c.iter() {
                    if let Some(v) = r.get(col) { v.hash_into(&mut h); }
                }
            }
            (h.finish(), content)
        }
...
impl SubscriptionManager {
    ...
    pub fn subscribe(&self, sub_id: &str, q: &str, cols: Option<Vec<String>>, mode: SubscriptionMode) -> Result<SubscribeResult, String> {
        ...
        match self.subs.entry(sub_id.clone()) {
            ...
            Entry::Vacant(entry) => {
                ...
                if !a.is_valid { return Err(a.error.unwrap_or_else(|| "Invalid query".into())); }
                if a.tables.is_empty() { return Err("No table in query".into()); }
                ...
                let is_new_query = match self.queries.entry(query_id.clone()) {
                    ...
                    Entry::Vacant(qe) => {
                        // New query
                        let cols_arc = cols.map(|c| Arc::from(c.into_iter().map(|s| Arc::<str>::from(s.as_str())).collect::<Vec<_>>().into_boxed_slice()));
                        
                        // Index by tables
                        for t in &a.tables {
                            self.table_idx.entry(Arc::<str>::from(t.as_str())).or_default().insert(query_id.clone());
                        }
    ...
    pub fn unsubscribe(&self, sub_id: &str) -> bool {
        let Some((_, sub)) = self.subs.remove(sub_id) else { return false };
    ...
    pub fn heartbeat(&self, sub_id: &str) -> bool {
        self.subs.get(sub_id).map(|s| *s.last_activity.write() = Instant::now()).is_some()
    }
    ...
    #[inline]
    pub fn get_sub(&self, sub_id: &str) -> Option<dashmap::mapref::one::Ref<'_, Arc<str>, Subscription>> {
        self.subs.get(sub_id)
    }
    
    #[inline]
    pub fn get_query(&self, query_id: &str) -> Option<dashmap::mapref::one::Ref<'_, Arc<str>, SharedQuery>> {
    ...
    where F: FnMut(&Arc<str>) {
        if let Some(qids) = self.table_idx.get(t) {
            for qid in qids.iter() { f(qid); }
        }
    }
...
impl Snapshot {
    #[inline] pub fn new() -> Self { Self { rows: FxHashMap::default() } }
    
    /// Initialize with typed rows - returns events for Events mode
    pub fn init_rows(&mut self, rows: Vec<RowData>, cols: &Option<Arc<[Arc<str>]>>) -> Vec<SubscribeEvent> {
        let t = ts();
        let len = rows.len();
        let mut ev = Vec::with_capacity(len);
        self.rows.clear(); self.rows.reserve(len);
        for row in rows {
            let (id_hash, content_hash) = row_hashes(&row, cols);
            let val = Arc::new(row.to_value());
            ev.push(SubscribeEvent::insert_arc(t, val.clone()));
            self.rows.insert(id_hash, RowEntry { hash: content_hash, data: val });
        }
        ev
    }

    /// Initialize with typed rows - returns row data for Snapshot mode
    pub fn init_rows_snapshot(&mut self, rows: Vec<RowData>, cols: &Option<Arc<[Arc<str>]>>) -> Vec<Arc<serde_json::Value>> {
        let len = rows.len();
        self.rows.clear(); self.rows.reserve(len);
    ...
    #[inline]
    pub fn diff_rows(&mut self, rows: Vec<RowData>, cols: &Option<Arc<[Arc<str>]>>) -> Vec<SubscribeEvent> {
        let t = ts();
        ...
        for row in rows {
            ...
            if let Some(prev) = old.remove(&id_hash) {
                ...
                if prev.hash == content_hash {
                    new.insert(id_hash, RowEntry { hash: content_hash, data: prev.data });
                } else {
                    ev.push(SubscribeEvent::delete_arc(t, prev.data));
                    let val = Arc::new(row.to_value());
                    ev.push(SubscribeEvent::insert_arc(t, val.clone()));
                    new.insert(id_hash, RowEntry { hash: content_hash, data: val });
                }
            } else {
                let val = Arc::new(row.to_value());
                ev.push(SubscribeEvent::insert_arc(t, val.clone()));
        ...
        ev.reserve(old.len());
        for (_, old_row) in old { ev.push(SubscribeEvent::delete_arc(t, old_row.data)); }
        self.rows = new;
        ev
    }
}
